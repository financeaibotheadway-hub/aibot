...
# ──────────────────────────────────────────────────────────────────────────────
# FIX FORMAT_DATE MISUSE (LLM SAFE)
# ──────────────────────────────────────────────────────────────────────────────
def fix_format_date(sql: str) -> str:
    """
    Fixes invalid FORMAT_DATE usage generated by LLM:
    - FORMAT_DATE(col)        ❌
    - FORMAT_DATE('%Y-%m')    ❌

    Strategy:
    - If FORMAT_DATE has < 2 arguments → replace with DATE(col) or col
    """
    ...
    def _fix(m: re.Match) -> str:
        ...
        # якщо є кома → аргументів >=2 → ок
        if "," in inside:
            return m.group(0)

        # якщо це колонка або вираз — просто DATE(...)
